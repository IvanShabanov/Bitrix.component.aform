...
/************************************** */
/************************************** */
if (!class_exists('aform_component')) {
    class aform_component {
        private $ENC;
        private $arrParams;
        ...
        function Save() {
            ...
            foreach ($arParams['FIELDS'] as $field) {
                if ($field['type'] == 'submit') {
                    ...
                    $uploaded = $this->SimpleUpload($field['name'], $_SERVER["DOCUMENT_ROOT"].'/upload/', false, $field['avalable_extensions']);
...
$form = new aform_component($arParams);
$arResult = $form->Autorun();
